...
const AddCardForm = ({ newCard, handleInputChange, handleAddCard }) => (
    <tr>
        ...
        <td><button className="add-btn">추가</button></td>
    </tr>
);
    ...
    return (
        <>
            ...
            <div className="payment-method">
                <h3>결제 관리</h3>
                <ul>
                    <li><button onClick={() => setActiveTab('card')} className={`method-menu ${activeTab === 'card' ? 'active' : ''}`}>카드결제</button></li>
                    <li><button onClick={() => setActiveTab('account')} className={`history-menu ${activeTab === 'account' ? 'active' : ''}`}>계좌이체</button></li>
                </ul>

                <div className="payment-section">
                    <h2>{activeTab === 'card' ? '카드 결제 정보' : '계좌 이체 정보'}</h2>
